...
@UserController.route('paginate/<search>', methods=['GET'])
@jwt_required()
@check_admin_permission("genin")
@paginate
def usersFirstnamePaginated(search , page_number, page_size):
    try:
        userPage = get_users_paginated_handler.handle(GetAllInput(search=search), page_number, page_size)
